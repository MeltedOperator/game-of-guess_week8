...
def main():
    ...
    attempts = 10
    played_games = 1 #сыгранные игры(не используется)
    ...
    savestats(attempts,start_number_attempts, difficulty, played_games, endgame_state)#выводим статистику
    play_again() 
...
def difficulty_choose(): #игрок здесь выбирает сложность. Тут тоже смотрим чтобы он ввел правильное значение
    while True:
        try:
            ...
            print("это даже не число...")
print("Привет! Давай поиграем? Я загадываю число, а" \
" ты пытаешься его угадать за определенное число попыток. Если тебе" \
"не удастся отгадать число, то ты проиграешь, если получится, ты победишь и я тебя похвалю." \
"Согласен? Для начала выбери уровень сложности, просто поставь цифру ниже:")
print("1 - Легкий: от 1 до 50. 10 попыток ")
print("2 - Средний: от 1 до 100. 7 попыток ")
print("3 - Сложный: от 1 до 1000. 10 попыток ")

def savestats(attempts_results, start_attempts_results, difficulty_results, number_played_games, endgame_condition): #здесь записываем статистику в файл stats2.txt
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open("stats2.txt", "a", encoding="utf-8") as f:
        f.write(f"Игра была: {endgame_condition} | {timestamp} | Осталось попыток: {attempts_results} из {start_attempts_results} возможных | "\
        f"Сложность: {difficulty_results} \n")
